...
# to first byte for websites from all servers except Beijing.
# Command: python getAllData.py (name of .txt file with websites) (name of CSV file)
...
csv_columns = ['url', 'PageSize', 'ResultLength', 'test', 'ohio', 'cal', 'oregon', 'canada', 'saopaulo', 'frankfurt', 'ireland', 'london', 'paris', 'mumbai', 'tokyo', 'seoul', 'singapore', 'sydney']
...
def getSinglePLTandTTFB(server, ID):
...
def getAllData():
    ...
    server_and_PLT_dict = {}
    for server in serverIDs:
        TTFBtime, pageLoadTime = getSinglePLTandTTFB(server, serverIDs[server])
...
def main():
    ...
    csvfile = open(sys.argv[2] + "_AllData_TTFB.csv", 'w')
    writer = csv.DictWriter(csvfile,fieldnames=csv_columns)
    writer.writeheader()

    csvfile2 = open(sys.argv[2] + "_ALLData_PLT.csv", 'w')
    ...
    for line in websites:
        ...
        URLandPLTDict['PageSize'], URLandPLTDict['ResultLength'] = getPSandRL(serverIDs["test"])
        URLandTTFBDict['PageSize'] =  URLandPLTDict['PageSize']
        URLandTTFBDict['ResultLength'] = URLandPLTDict['ResultLength']
